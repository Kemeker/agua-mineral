import React from "react"
import Logo from '../assets/fiveicon.png'

function Section01(){
    return(
        <section className="bg-black py-12">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center">
                <h2 className="text-base text-indigo-600 font-semibold tracking-wide uppercase">Bem Vindo a Fonte de Água Jupiá</h2>
                <p className="mt-2 text-3xl leading-8 font-extrabold tracking-tight text-gray-900 sm:text-4xl">
                    A Melhor água das americas.
                </p>
                <p className="mt-4 max-w-2xl text-xl text-gray-500 lg:mx-auto">
                    Lorem ipsum dolor sit amet consectetur adipisicing elit. Possimus magnam voluptatum cupiditate veritatis in, accusamus quisquam.
                </p>
                </div>

                <div className="mt-10">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    <div className="p-6 max-w-sm mx-auto bg-white rounded-xl shadow-md flex items-center space-x-4">
                    <div className="shrink-0">
                        <img className="h-12 w-12" src={Logo} alt="Workflow" />
                    </div>
                    <div>
                        <div className="text-xl font-medium text-black">Quem somos</div>
                        <p className="text-gray-500">Get the latest updates from us.</p>
                    </div>
                    </div>

                    <div className="p-6 max-w-sm mx-auto bg-white rounded-xl shadow-md flex items-center space-x-4">
                    <div className="shrink-0">
                        <img className="h-12 w-12" src={Logo} alt="Workflow" />
                    </div>
                    <div>
                        <div className="text-xl font-medium text-black">Produtos</div>
                        <p className="text-gray-500">We're here to help you at any time.</p>
                    </div>
                    </div>

                    <div className="p-6 max-w-sm mx-auto bg-white rounded-xl shadow-md flex items-center space-x-4">
                    <div className="shrink-0">
                        <img className="h-12 w-12" src={Logo} alt="Workflow" />
                    </div>
                    <div>
                        <div className="text-xl font-medium text-black">Fonte</div>
                        <p className="text-gray-500">Join us to get more exclusive offers.</p>
                    </div>
                    </div>
                </div>
                </div>
            </div>
        </section>


    )
}

export default Section01